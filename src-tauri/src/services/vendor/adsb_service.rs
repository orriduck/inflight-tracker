...
impl AdsbService {
    ...
    pub async fn get_data(&self, callsign: Option<&str>) -> Result<FlightData, AdsbError> {
        ...
        if !response.status().is_success() {
            log::error!(
                "ADSB API returned status: {} for callsign: {}",
                response.status(),
                callsign
            );
            return Err(AdsbError::RequestError(reqwest::Error::from(
    ...
    pub async fn get_recommend_callsigns(
        ...
        if !response.status().is_success() {
            log::error!(
                "ADSB nearby callsigns API returned status: {}",
                response.status()
            );
            return Err(AdsbError::RequestError(reqwest::Error::from(
                response.error_for_status().unwrap_err(),
    ...
    fn convert_v2_aircraft_to_flight_data(
        &self,
        aircraft: &Value,
        callsign: &str,
    ) -> Result<FlightData, AdsbError> {
